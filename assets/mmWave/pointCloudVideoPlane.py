...
def animate_point_cloud_folder(folder_path, file_prefix):
    ...
    def update(frame):
        ...
        # 更新XOZ平面散点图数据点位置和颜色
        ax_xoz.scatter(x, z, c=snr, cmap='rainbow')
        ax_xoz.set_title('XOZ Plane')

        # 更新YOZ平面散点图数据点位置和颜色
        ax_yoz.scatter(y, z, c=snr, cmap='rainbow')
        ax_yoz.set_title('YOZ Plane')

    # 设置坐标轴范围
    ax_xoz.set_xlim(2, 8)  # X 轴范围为 0 到 15
    ax_xoz.set_ylim(2, 8)  # Z 轴范围为 0 到 15

    ax_yoz.set_xlim(0.5, 2)  # Y 轴范围为 0 到 10
    ax_yoz.set_ylim(2, 8)  # Z 轴范围为 0 到 15
